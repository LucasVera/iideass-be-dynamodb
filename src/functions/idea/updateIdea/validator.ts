...
export const validateInput = (body: FromSchema<typeof schema>) => {
  const { email, subject, description, ideaType } = body
  ...
  if (!description && !ideaType)
    throw new BadInputError('At least one field should be specified for update: description or type', {
      description,
      ideaType,
    })
